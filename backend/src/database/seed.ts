import { PrismaClient, Role, StatusPeserta, TipeAbsensi, StatusAbsensi, TipeIzin, StatusPengajuan } from '@prisma/client';
...
async function main() {
  ...
  await prisma.absensi.create({
    data: {
      ...
      timestamp: new Date(yesterday.getTime() + 8 * 60 * 60 * 1000).toISOString(), // 8 hours later
      lokasi: {
        latitude: -6.2088,
        longitude: 106.8456,
        address: 'Jakarta, Indonesia'

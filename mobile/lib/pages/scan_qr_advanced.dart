...
class ScanQRAdvancedPage extends StatefulWidget {
  const ScanQRAdvancedPage({super.key});

  @override
  State<ScanQRAdvancedPage> createState() => _ScanQRAdvancedPageState();
}

class _ScanQRAdvancedPageState extends State<ScanQRAdvancedPage> {
  ...
  Future<void> _authenticateWithBiometric() async {
    try {
      ...
      if (!isAvailable) {
        _showErrorDialog('Autentikasi biometrik tidak tersedia');
        return;
      }

      final success = await BiometricService.authenticateForAttendance();
      if (success) {
        _showSuccessDialog('Autentikasi Berhasil', 'Autentikasi biometrik berhasil dilakukan.');
      } else {
        _showErrorDialog('Autentikasi biometrik gagal');
      }
    } catch (e) {
      _showErrorDialog('Error autentikasi: ${e.toString()}');
    }
  }
